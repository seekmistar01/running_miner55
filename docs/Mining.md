...
To participate as a Miner in the NIOME subnet and earn **TAO (**$\tau$**)** emissions, you must first set up your machine, install the necessary software, and register your identity on the Bittensor blockchain.
...
3. **Create a Hotkey (Miner Identity):** The hotkey is used to sign transactions, run the miner, and receive emissions. It is connected to your coldkey.
   **Bash**
   ...
   ```
   btcli wallet new_hotkey --wallet.name niome_wallet --wallet.hotkey niome_miner
   ```
...
5. **Register Your Hotkey to Subnet:** Register your hotkey to secure a UID (Unique Identifier) on the NIOME subnet. The Network ID for NIOME is xx.
   **Bash**
   ...
   ```
   btcli subnet register --netuid xx --wallet.name niome_wallet --wallet.hotkey niome_miner
   ```
    In case of testnet

    ```
   btcli subnet register --netuid 289 --network test --wallet.name niome_wallet --wallet.hotkey niome_miner
   ```
...
Once your hotkey is registered, you can start your Miner. The parameters you pass will determine your specific role (Architect, Adversary, or Oracle) and network configuration.
