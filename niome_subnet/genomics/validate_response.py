...
def _validate_vcf_metadata(
    miner_vcf: str, task: GenomicSimulationTask
) -> Dict[str, Any]:
    validation_result = {
        "valid": False,
        "extracted_metadata": [],
    ...
    try:
        ...
        if not extracted_metadata:
            validation_result["mismatches"].append(
                "No metadata extracted from VCF headers"
            )
            return validation_result
        validation_result = _compare_task_metadata(
            extracted_metadata, validation_result, task
        )
    except Exception as e:
